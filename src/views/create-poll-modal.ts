...
app.view("create-poll-view", async ({ ack, view, client }) => {
  ...
  const message = createPollMessage({
    channelId: view.state.values.channel.channels_select.selected_channel as string,
    options: [
  ...
  const messageResponse = await client.chat.postMessage(message).catch(async err => {
